...
minmax x y z = 
    let
        low = min x (min y z) -- Searched on the internet and wanted to add different options.
        high = maximum [x, y, z]
    in
        high - low
...
lowerAndGreater n list =
    let 
        values = lowerHigher n list
    in
    show n ++ " is greater than " ++ show (fst values)  ++ " elements and lower than " ++ show (snd values) ++ " elements"

lowerHigher :: Int-> [Int]-> (Int, Int)
lowerHigher val = foldl lowerGreater (0, 0) 
    where lowerGreater (l, h) x | x < val = (l+1, h) 
          lowerGreater (l, h) x | x > val = (l, h+1) 
          lowerGreater (l, h) _ = (l, h)